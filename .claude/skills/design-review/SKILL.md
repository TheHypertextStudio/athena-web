...
6. **Write the scorecard** to `docs/design/audits/YYYY-MM-DD-<surface>.md`. Open the file with the machine-readable front-matter block (`surfaces`, `date`, `verdict`, all eight `scores`, all five `gates`) — the schema, the id vocabulary, and the ✅/❌ mapping are documented in `docs/design/audits/README.md`, and the surface ids must already exist in `docs/design/surface-inventory.md` (regenerate it with `pnpm exec tsx scripts/surface-inventory.ts` if the surface is new). A scorecard with missing or invalid front matter fails `packages/test-utils/tests/design-policies/scorecard-schema.test.ts`, and `verdict: ship` is rejected outright unless all five gates are true and every dimension is at or above the bar. Then write the body using the format in the rubric, including screenshot paths and findings ordered by severity with proposed fixes.
