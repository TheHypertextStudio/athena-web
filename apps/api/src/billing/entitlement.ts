 ...
 * refuses with a typed 402 the web app renders as a targeted upsell.
 *
 ...
 * @throws {AgentPlanRequiredError} When the org's lifecycle state is not entitled.
...
export async function assertAgentSessionsEntitled(orgId: string): Promise<void> {
  const rows = await db
    .select({ lifecycleState: organization.lifecycleState })
    .from(organization)
