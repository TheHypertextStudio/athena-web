/**
 * `domain packages` — the RFC 9457 problem model.
 *
 * @remarks
...
export const ProblemCode = z
  ...
  .describe(
    [
      ...
      '- `image_in_use` (HTTP 409): saved prose still references an uploaded image, so its bytes cannot be removed.',
      '- `identity_in_use` (HTTP 409): a linked identity still funds one or more external connections and must not be removed.',
      '- `external_identity_mismatch` (HTTP 409): an external Athena continuation does not match an identity linked by the current user.',
      ...
      '- `calendar_source_scope_required` (HTTP 409): removing an upstream calendar subscription requires incremental provider consent.',
      '- `calendar_source_protected` (HTTP 409): the calendar is primary, owned, or otherwise cannot be removed as a subscription.',
      '- `calendar_source_writes_pending` (HTTP 409): pending writes or unresolved conflicts must finish before the calendar source can be removed.',
      '- `calendar_source_removal_failed` (HTTP 503): the provider did not confirm source removal, so local state remains unchanged.',
      ...
      '- `rate_limited` (HTTP 429): this credential has made too many requests in its current window — wait and retry.',
      '- `geocoding_unavailable` (HTTP 503): saved-place address search is unavailable — retry without exposing provider details.',
      ...
      '- `billing_unavailable` (HTTP 503): Docket has paused new Checkout sessions and discount applications while existing billing remains available.',
      '- `phone_verification_unavailable` (HTTP 503): Docket cannot start or resend a phone verification for this account right now.',
...
export const PUBLIC_PROBLEM_TITLES = {
  ...
  cadence_changed: 'Cycle settings changed. Refresh and try again.',
  image_in_use: 'That image is still used in saved content.',
  identity_in_use: 'That connected account is still in use.',
  external_identity_mismatch: 'Connect the account that opened this Athena session.',
  ...
  calendar_source_scope_required: 'Connect this account with calendar-list access to continue.',
  calendar_source_protected: 'This calendar cannot be removed from its account.',
  calendar_source_writes_pending: 'Resolve pending calendar changes before removing this source.',
  calendar_source_removal_failed: 'The calendar provider did not remove this source.',
  ...
  geocoding_unavailable: 'Address search is unavailable.',
  ...
  billing_customer_missing: 'Billing needs staff attention before you can continue.',
  billing_unavailable: 'New billing changes are paused.',
  phone_verification_unavailable: 'Phone verification is not available right now.',
  checkout_pending: 'Docket is already preparing Checkout.',
  subscription_exists: 'Docket Pro is already active for this workspace.',
  discount_application_pending: 'This workspace already has a discount application in review.',
  discount_award_conflict: 'This workspace already has a discount award.',
  billing_provider_sync_failed: 'Docket could not confirm this billing change.',
  billing_grace_expired: 'The payment recovery period has ended.',
...
export const PUBLIC_PROBLEM_SUMMARIES = {
  ...
  cadence_changed: 'The team cadence changed after you loaded it.',
  image_in_use: 'Remove the image from saved content before deleting it.',
  identity_in_use: 'At least one connection still depends on this account.',
  external_identity_mismatch: 'The linked account does not match this external Athena request.',
  account_selection_required: 'More than one connected account can handle this request.',
  linear_workspace_already_connected:
    'That Linear workspace belongs to an existing Docket connection.',
  linear_write_scope_required: 'The current Linear connection cannot update Linear.',
  calendar_source_scope_required: 'The current calendar grant cannot change source membership.',
  calendar_source_protected: 'Primary and owner-managed calendars must stay on their accounts.',
  calendar_source_writes_pending: 'This source still has changes that could be lost.',
  calendar_source_removal_failed: 'No local calendar data changed. Try the removal again.',
  task_already_linked: 'Docket already has this relationship.',
  idempotency_key_reuse: 'The same request key was used with different information.',
  dependency_cycle: 'The requested link would make the task depend on itself.',
  last_owner_guard: 'This change would leave the workspace without an owner.',
  current_session: 'The session making this request must remain active.',
  self_escalation: 'People cannot grant themselves more access.',
  personal_org_no_invites: 'A personal workspace has one member.',
  reauth_required: 'The current session is too old for this sensitive action.',
  rate_limited: 'Too many requests. Try again shortly.',
  geocoding_unavailable: 'Docket could not complete the address lookup. Try again shortly.',
  deletion_blocked: 'You still own a shared workspace that must be transferred or deleted.',
  card_required: 'Docket cannot start this purchase without payment details.',
  billing_frozen: 'Past-due or canceled billing currently blocks this change.',
  billing_customer_missing: 'Docket could not match this workspace to one Stripe customer.',
  billing_unavailable: 'Docket has paused new purchases and discount applications.',
  phone_verification_unavailable: 'No verification request was created. Try again later.',
  checkout_pending: 'Another Checkout request for this workspace is still in progress.',
  subscription_exists: 'This workspace already has a current Docket Pro subscription.',
  discount_application_pending: 'Docket must finish the current application before another starts.',
  discount_award_conflict: 'Discounts cannot stack on one workspace.',
  billing_provider_sync_failed: 'Stripe did not confirm the requested billing change.',
  billing_grace_expired: 'Shared work is read-only until payment recovers.',
  product_required: 'This organization does not have Docket Pro.',
  agent_plan_required: 'This organization does not have Docket Pro.',
  domain_already_claimed: 'Another workspace already uses this domain.',
  public_name_taken: 'Another public page already uses this address.',
  method_not_allowed: 'This address does not accept the requested method.',
  not_acceptable: 'This address cannot return the requested response format.',
...
const PROBLEM_RECOVERY: Record<ProblemCode, ProblemRecovery> = {
  ...
  not_found: 'return',
  conflict: 'review',
  cadence_changed: 'retry',
  image_in_use: 'review',
  identity_in_use: 'review',
  external_identity_mismatch: 'reconnect',
  account_selection_required: 'review',
  linear_workspace_already_connected: 'review',
  linear_write_scope_required: 'reconnect',
  calendar_source_scope_required: 'reconnect',
  calendar_source_protected: 'review',
  calendar_source_writes_pending: 'review',
  calendar_source_removal_failed: 'retry',
  ...
  billing_customer_missing: 'billing',
  billing_unavailable: 'retry',
  phone_verification_unavailable: 'retry',
  checkout_pending: 'retry',
  subscription_exists: 'billing',
  discount_application_pending: 'review',
  discount_award_conflict: 'review',
  billing_provider_sync_failed: 'retry',
  billing_grace_expired: 'billing',
...
const PROBLEM_STATUS: Record<ProblemCode, number> = {
  ...
  image_in_use: 409,
  identity_in_use: 409,
  external_identity_mismatch: 409,
  account_selection_required: 409,
  linear_workspace_already_connected: 409,
  linear_write_scope_required: 409,
  calendar_source_scope_required: 409,
  calendar_source_protected: 409,
  calendar_source_writes_pending: 409,
  calendar_source_removal_failed: 503,
  ...
  card_required: 402,
  billing_frozen: 402,
  billing_customer_missing: 409,
  billing_unavailable: 503,
  phone_verification_unavailable: 503,
  checkout_pending: 409,
  subscription_exists: 409,
  discount_application_pending: 409,
  discount_award_conflict: 409,
  billing_provider_sync_failed: 409,
  billing_grace_expired: 402,
