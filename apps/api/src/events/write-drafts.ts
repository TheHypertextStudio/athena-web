...
/** One draft, resolved against Docket identities and ready to insert. */
interface PreparedDraft {
  ...
  readonly association: DraftAssociation;
  readonly externalRecipients: Map<string, StreamRelevance>;
  readonly actorRef: ActorRef | null;
  readonly participantRefs: ActorRef[];
}

/** What one successful insert produced. */
interface WrittenEvent {
  readonly eventId: string;
  readonly recipients: Map<string, StreamRelevance>;
}

/**
 * Resolve a draft's Docket-side references ahead of the write.
 *
 * @remarks
 * Actor and recipient lookups are read-only, so they happen outside the transaction rather than
 * holding it open for the length of an identity join.
 *
 * @param draft - The normalized draft.
 * @param ctx - The tenancy and provenance the whole batch shares.
 * @param resolvedEntities - Docket ids resolved for this whole batch.
 * @returns The prepared draft, or `undefined` when its kind is not a known enum value.
 */
async function prepareDraft(
  ...
  const { organizationId: orgId, sourceSystem: source } = ctx;
  ...
  return {
    ...
    entityKind: draft.entity?.kind ?? null,
    entityRef: toEntityRef(draft.entity, source),
    association: associationFor(draft.entity, resolvedEntities),
    // Resolve mentioned external users → linked Docket users, so the mention routes to whoever was
    // actually named (the integration-owner fallback still applies for unlinked participants).
    externalRecipients: await resolveLinkedIdentityRecipients(
      source,
      draft.participants,
      kind.data,
    ),
    actorRef: await toActorRef(orgId, draft.actor, source),
    participantRefs,
  };
}

/**
 * Insert one event and fan it out to its recipients, in one transaction.
 *
 * @param prepared - The resolved draft.
 * @param ctx - The tenancy and provenance the whole batch shares.
 * @returns The written event, or `null` when the `dedupeKey` was already recorded.
 */
async function insertDraftEvent(
  prepared: PreparedDraft,
  ctx: DraftWriteContext,
): Promise<WrittenEvent | null> {
  const { draft, kind, occurredAt, entityKind, entityRef, association } = prepared;
  ...
  return db.transaction(async (tx) => {
    ...
    const recipients = await routeAndWriteRecipients(
      ...
      row.id,
      {
        ...
        ownerUserId: userId,
        externalRecipients: prepared.externalRecipients,
      },
      occurredAt,
    );
...
/**
 * Reindex, publish, and run automations for an event that was just written.
 *
 * @param prepared - The resolved draft the event came from.
 * @param written - The inserted event and its recipients.
 * @param ctx - The tenancy and provenance the whole batch shares.
 */
async function announceWrittenEvent(
  prepared: PreparedDraft,
  written: WrittenEvent,
  ctx: DraftWriteContext,
): Promise<void> {
  const { draft, kind, occurredAt, entityKind, entityRef, association } = prepared;
  const { organizationId: orgId, userId, sourceSystem: source } = ctx;
  ...
  const recipients = [...written.recipients].map(([uid, reason]) => ({ userId: uid, reason }));
  await publishEvent(written.eventId, recipients).catch(() => undefined);
...
export async function writeEventDrafts(
  ...
  const resolvedEntities: ResolvedEntities = ctx.integrationId
    ? await resolveExternalEntities(
        { organizationId: ctx.organizationId, integrationId: ctx.integrationId },
  ...
  for (const draft of drafts) {
    const prepared = await prepareDraft(draft, ctx, resolvedEntities);
    if (!prepared) continue; // skip drafts whose kind isn't a known enum value
    const written = await insertDraftEvent(prepared, ctx);
    if (!written) continue;

    created += 1;
    if (prepared.association.state === 'matched') associated += 1;
    recipientsWritten += written.recipients.size;
    await announceWrittenEvent(prepared, written, ctx);
  }
