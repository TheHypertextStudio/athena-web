...
import { optionCatalog } from './facet-catalog';
...
/** One facet bucket's request: the field to count, and who is asking about it. */
interface FacetBucketInput {
  database: Database;
...
/** The page the caller asked for within one facet bucket. */
interface FacetPage {
...
/**
 * The statement that counts one facet's options and pages through them.
 *
 * @remarks
 * Every option the caller could pick appears, whether or not it matches anything right now: the
 * catalog is unioned with the counted memberships, so a filter's picker never hides a value just
 * because the current view has none of it.
 *
 * @param input - The facet bucket being read.
 * @param page - The page within that bucket.
 * @returns The aggregate query.
 */
function facetAggregateSql(input: FacetBucketInput, page: FacetPage): SQL {
  const { asOf, search, cursorKey } = page;
  const ctes = compileRosterCtes({
    ...
    userId: input.userId,
    filter: activeFilter(input.request, input.field, input.actorId, new Date(asOf), input.timeZone),
  });
  ...
  return sql`with recursive ${ctes}${initiativeCatalogCtes}, memberships as materialized (
      ...
      where (${search}::text is null or available.label ilike ('%' || ${search} || '%'))
        and (${cursorKey}::text is null or available.key > ${cursorKey})
...
async function facetBucket(input: FacetBucketInput) {
  ...
  const aggregate = await executeOne(
    input.database,
    facetAggregateSql(input, { asOf, search, cursorKey: cursor?.key ?? null }),
    facetAggregate,
  );
