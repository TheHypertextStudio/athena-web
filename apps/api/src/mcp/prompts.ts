...
const SYSTEM_PROMPT = [
  ...
  'update (change work by describing which work, not by listing ids, including its labels), link,',
  'archive, comment, report_status, plan_day, define_labels (create or edit labels and label',
  'groups), define_template (create or edit a template). Every one of them is reversible with',
  'undo, and every one is',
