...
export const adminBillingRoutes = new Hono<AppEnv>()
  .post('/:id/holds', zParam(idParam), zJson(PlaceHoldBody), async (c) => {
  ...
  .delete('/:id/holds/:holdId', zParam(holdParam), async (c) => {
  ...
  .post('/:id/reactivate', requireStaffRole('finance'), zParam(idParam), async (c) => {
    const { id } = c.req.valid('param');
    const { staffUserId } = c.get('staffCtx');
    const org = await loadOrg(id);
