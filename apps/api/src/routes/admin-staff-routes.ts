...
export const adminStaffRoutes = new Hono<AppEnv>()
  .get(
    ...
    apiDoc({ tag: 'Admin', summary: 'List staff members', response: AdminStaffPage }),
  ...
  .post(
    '/',
    requireStaffRole('superadmin'),
    apiDoc({ tag: 'Admin', summary: 'Grant staff access', response: AdminStaffOut }),
  ...
  .delete(
    '/:id',
    requireStaffRole('superadmin'),
    apiDoc({ tag: 'Admin', summary: 'Revoke staff access', response: AdminStaffOut }),
    zParam(staffParam),
    async (c) => {
