...
function defaultPriceKey(): string {
  /* eslint-disable @typescript-eslint/no-deprecated -- one-release Stripe configuration compatibility */
  return (
...
const billing = new Hono<AppEnv>()
  ...
  .post(
    ...
    apiDoc({
      ...
      description: `Open hosted checkout for Docket Pro and return {@link RedirectOut} \`{ url }\`. Docket sets the return URLs, price, and 14-day first-purchase trial; the optional \`customerEmail\` only pre-fills checkout. Modern Docket Pro configuration is preferred, with the former Docket Team configuration accepted temporarily for compatibility.

Side effect: creates a checkout session with the provider; product access changes only after a provider webhook confirms the subscription. Requires \`manage\`. Related: \`POST /portal\`, \`GET /\`, and \`GET /lifecycle\`.`,
    }),
    zJson(CheckoutBody),
    async (c) => {
  ...
  .post(
    ...
    apiDoc({
      ...
      capability: 'manage',
      response: RedirectOut,
      description: `Open the provider's hosted billing portal and return {@link RedirectOut} \`{ url }\`, where an administrator can update payment, cancel Docket Pro, and view invoices. Provider webhooks apply any resulting product and lifecycle changes. Requires \`manage\`.`,
    }),
    async (c) => {
      const { orgId } = c.get('actorCtx');
