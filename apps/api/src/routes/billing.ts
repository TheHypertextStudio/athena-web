...
const billing = new Hono<AppEnv>()
  .get(
    '/',
    apiDoc({ tag: 'Billing', summary: 'Get the org subscription', response: SubscriptionOut }),
    async (c) => {
  ...
  .get(
    '/lifecycle',
    apiDoc({ tag: 'Billing', summary: 'Get the org lifecycle status', response: LifecycleOut }),
    async (c) => {
      const { orgId } = c.get('actorCtx');
