...
const grants = new Hono<AppEnv>()
  ...
  .put('/', capabilityGuard('manage'), zJson(GrantUpsert), async (c) => {
  ...
  .delete('/:grantId', capabilityGuard('manage'), zParam(grantIdParam), async (c) => {
    const { orgId } = c.get('actorCtx');
    const { grantId } = c.req.valid('param');
    const deleted = await db
      .delete(grant)
      .where(and(eq(grant.id, grantId), eq(grant.organizationId, orgId), eq(grant.effect, 'allow')))
      .returning();
    const row = deleted[0];
