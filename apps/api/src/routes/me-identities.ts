...
const meIdentities = new Hono<AppEnv>()
  .get(
    ...
    apiDoc({
      ...
      description: `List the Google, GitHub, and Linear accounts the caller linked to Docket. A linked identity belongs to the person. An organization integration separately chooses one of those identities and the external resources to synchronize.

Each item includes the provider's stable \`accountId\`, granted \`scopes\`, link time, and \`connectionCount\`, which is the number of organization integrations that depend on it. \`email\`, \`name\`, and \`picture\` are optional because not every provider or token supplies them. Use \`GET /v1/me/connected-apps\` to list third-party apps that the caller authorized to access Docket.`,
    }),
    async (c) => {
