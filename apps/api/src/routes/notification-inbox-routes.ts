  ...
  const router = new Hono<AppEnv>()
    .get(
      ...
      async (c) => {
        ...
        const page = await inbox.list(session.user.id, c.req.valid('query'));
    ...
    .get(
      ...
      async (c) => {
        const session = c.get('session');
        if (!session?.user) throw new AuthError();
        return ok(c, NotificationCount, await inbox.count(session.user.id));
      },
    )
    .post(
      '/read-all',
      ...
      async (c) => {
        const session = c.get('session');
        if (!session?.user) throw new AuthError();
        const result = await inbox.readAll(session.user.id, c.req.valid('json'));
  ...
  if (options.includeDetail) {
    router.get(
      ...
      async (c) => {
        const session = c.get('session');
        if (!session?.user) throw new AuthError();
        const notification = await inbox.get(session.user.id, c.req.valid('param').id);
  ...
  return router
    .post(
      ...
      async (c) => {
        const session = c.get('session');
        if (!session?.user) throw new AuthError();
        const notification = await inbox.markRead(session.user.id, c.req.valid('param').id);
        if (!notification) throw new NotFoundError('Notification not found');
        return ok(c, NotificationOut, notification);
      },
    )
    .post(
      ...
      async (c) => {
        const session = c.get('session');
        if (!session?.user) throw new AuthError();
        const notification = await inbox.act(session.user.id, c.req.valid('param').id);
        if (!notification) throw new NotFoundError('Notification not found');
        return ok(c, NotificationOut, notification);
      },
    );
