...
const projects = new Hono<AppEnv>()
  .post(
    ...
    apiDoc({
      ...
      response: ProjectOut,
      description: `Create a project in the organization. Projects move from \`planned\` to \`active\` and then \`completed\`, or to \`canceled\`. The status defaults to \`planned\`. \`startDate\` and \`targetDate\` use ISO dates. Optional \`leadId\`, \`teamId\`, \`programId\`, and every \`initiativeIds\` value must identify a resource in the same organization; otherwise the request returns 404. Duplicate initiative IDs are accepted once. Optional \`milestones\` are created in their supplied order unless an item sets \`sort\`. The project, initiative links, and milestones succeed together or fail together. Docket records the creation in activity. Requires \`contribute\` and returns the created {@link ProjectOut}. Use \`GET /:id/progress\` for completion.`,
    }),
    zJson(ProjectCreate),
    async (c) => {
  ...
  .patch(
    ...
    apiDoc({
      ...
      capability: 'contribute',
      response: ProjectOut,
      description: `Update selected project fields. Omitted fields remain unchanged, while null clears a nullable field. A supplied \`leadId\`, \`programId\`, or \`teamId\` must identify a resource in the same organization; otherwise the request returns 404. An empty body leaves the project unchanged. Changing \`programId\` moves the project under that program. Status, health, and date changes affect project roll-ups and initiative timelines. Docket records a status-change activity only when \`status\` is present. Requires \`contribute\` and returns the updated {@link ProjectOut}.`,
    }),
    zParam(idParam),
    zJson(ProjectUpdate),
    async (c) => {
  ...
  .get(
    ...
    apiDoc({
      ...
      description: `Return completion progress across the project's tasks that the caller can view. A task is complete when \`completedAt\` is set. If any visible task has a positive \`estimate\`, Docket weights progress by estimates and treats a missing estimate as zero. Otherwise, each task has equal weight. \`percent\` is zero when no visible task contributes weight. \`taskCount\` and \`completedCount\` always report raw counts. An absent or inaccessible project returns 404. Returns {@link ProjectProgress}.`,
    }),
    zParam(idParam),
    async (c) => {
      const { orgId, actorId } = c.get('actorCtx');
