...
const stream = new Hono<AppEnv>()
  .get(
    ...
    apiDoc({
      ...
      description: `List workspace events newest first. Each event identifies its source, including Docket, Linear, and GitHub, and includes provider-neutral display fields. Unlike the personal stream, this workspace stream includes all visible workspace activity and returns \`relevance: null\`.

\`system\`, \`kind\`, \`filter\`, and \`viewId\` filters combine with AND. Paging uses \`cursor\`, \`limit\`, and \`order\`; the response includes \`nextCursor\` only when another page exists. Keep the same filters when continuing a page. Any workspace member may read the stream. Related: \`GET /v1/orgs/:orgId/activity\` returns Docket activity records rather than connected-system events.`,
    }),
    zQuery(StreamQuery),
    async (c) => {
