 ...
 *
 * @remarks
 * These tests exercise the organization, entitlement, billing-account, and complimentary-access
 * boundary. Bootstrapping those tables directly keeps root test runs parallel without paying for
 * every package migration.
 */
export async function createBillingLifecycleDb(): Promise<BillingLifecycleDbFixture> {
  ...
  await client.exec(`
    ...
    create unique index "billing_exemption_org_active_uq"
      on "billing_exemption" (organization_id)
