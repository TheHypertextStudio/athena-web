...
import { db, oauthClient, user } from '@docket/db';
...
describe('CIMD client metadata validation', () => {
  ...
  it('does not re-enable a disabled CIMD client when its metadata is refreshed', async () => {
    const clientId = 'https://allowed.example/disabled-client.json';
    const original = await cimd.resolveCimdClient(
      clientId,
      deps({
        client_id: clientId,
    ...
    await cimd.upsertCimdClient(original);
    const [owner] = await db
      .insert(user)
      .values({
        email: `disabled-cimd-${randomUUID()}@example.test`,
        name: 'Disabled CIMD owner',
      })
      .returning({ id: user.id });
    expect(owner).toBeDefined();
    const legacyBefore = new Date('2026-09-01T00:00:00.000Z');
    await db
      .update(oauthClient)
      .set({
    ...
    const refreshed = await cimd.resolveCimdClient(
      clientId,
      deps({
        client_id: clientId,
        client_name: 'Renamed while disabled',
        redirect_uris: ['https://allowed.example/new-callback'],
      }),
    );
    await cimd.upsertCimdClient(refreshed);
