...
describe('work-view routes', () => {
  ...
  it('rebalances a fixed neighborhood inside 50,000 rows and preserves window continuation', async () => {
    const { orgId, teamId, humanActorId, statusId } = await seedBaseOrg(schema.db, schema);
    await grantOrganizationCapability(orgId, humanActorId, 'contribute');
    ...
    await schema.db
      .execute(sql`update work_item_order set rank=${exhaustedRank}, updated_at=${oldWrite}
