...
<Update label="2026-09-12" description="Public REST contract identity" tags={["Developers"]}>
  ## REST clients can identify the contract they received

The public REST surface now publishes the prerelease `0.1.0` compatibility contract. Clients may
omit `Docket-Version` or assert that exact version. Every public REST response identifies the
contract and deployed source revision.

This change establishes response identity. It does not claim that every documented API capability
started on this date. Docket still publishes no SDK or outbound webhooks, and it supports one
current public contract.

Read [API versions](/developers/api-versions) before pinning a client.
...
API is prerelease, has no SDK, and publishes one current compatibility contract.
