...
`Project` and `Task` keep the same word in all three presets.
...
So these docs and the API agree; reading these docs beside the app is what needs the tables above.

Changing a preset moves, renames, and re-parents nothing. Every rule in
[Core concepts](/guides/concepts/workspaces) still holds: a Cycle called a Sprint is still
team-scoped and still a fixed window of time.

<Note>
  Set terminology per workspace. Your personal space keeps its own setting under the same
