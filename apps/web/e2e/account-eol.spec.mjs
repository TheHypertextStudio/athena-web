...
test.describe('account end-of-life', () => {
  test('exports account data as a downloadable ZIP', async ({ page }) => {
    ...
    await page.goto(`/orgs/${orgId}/settings/export`);
    await page.getByRole('button', { name: 'Request export' }).click();

    // The in-process dev sweep readies the export within a few seconds.
    await expect(page.getByText('Your export is ready')).toBeVisible({ timeout: 45_000 });
    ...
    expect(res.headers()['content-disposition']).toMatch(/attachment; filename="docket-export-.+\.zip"/);
  ...
  test('schedules then cancels recoverable account deletion', async ({ page }) => {
    ...
    await page.goto(`/orgs/${orgId}/settings/danger`);
