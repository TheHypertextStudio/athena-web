 ...
 *
 * Signs up a real throwaway user, opens the nested Calendar configuration surface, and exercises
 * the browser flow against deterministic Calendar API responses. Google OAuth itself is outside
 * this test's boundary; the mocked `/v1/me/calendar` + `/v1/agenda` responses stand in for linked
 * Google accounts/events while keeping the shell, routing, TanStack Query, and rendering real.
 */
...
test('Google Calendar config is nested and feeds the agenda rail', async ({ page }) => {
  ...
  await page.route('**/v1/agenda?**', async (route) => {
    await route.fulfill({ json: selected ? agendaPayload() : { date: '2026-06-30', entries: [] } });
  });

  await page.goto(`/orgs/${orgId}/settings/connections`, { waitUntil: 'domcontentloaded' });
  await expect(page.getByRole('link', { name: /Google Calendar/ })).toBeVisible();
  await page.getByRole('link', { name: /Google Calendar/ }).click();

  await expect(page).toHaveURL(new RegExp(`/orgs/${orgId}/settings/connections/google-calendar`));
  ...
  await page.goto(`/orgs/${orgId}/today`, { waitUntil: 'domcontentloaded' });
