/**
 * The org Teams list — server entry (SSR prefetch + hydration).
 *
 * @remarks
 * Prefetches the roster's slices (teams, projects, tasks) with the caller's session cookie,
 * dehydrates them, and hands the warm cache to {@link TeamsListClient} via `<HydrationBoundary>` —
 * so the table paints from data on first load, not a skeleton. A failed prefetch degrades
 * gracefully (the client fetches that slice). See `docs/engineering/specs/data-layer.md` §7.
 */
