 ...
 * authentication ceremony via `authClient.signIn.passkey()` — no email, no password. Beneath it,
 * {@link OAuthSignIn} offers whichever identity providers this deployment has actually configured
 * (none on the local stack, where the whole block is absent). Passkey-*first*, not passkey-only:
 * bringing an account you already have is a real way in, and it is the only one available on a
 * device with no passkey enrolled and no WebAuthn support. Where the
 ...
 *   passkey button is disabled. {@link OAuthSignIn} then matters most: it is the way into an
 *   account from a browser or device that cannot do WebAuthn at all.
 ...
 *
 * This is the client half of `/sign-in`; the route's `page.tsx` is a Server Component that already
 * redirected an authenticated visitor away before any of this reached the browser. The
 * `useRedirectIfAuthenticated` check below remains as the belt-and-braces guard for a session that
 * appears *after* mount — a second tab signing in, or a ceremony completing elsewhere — which no
 * server-render check can cover.
 *
