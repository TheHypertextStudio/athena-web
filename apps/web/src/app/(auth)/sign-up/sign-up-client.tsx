 ...
 *
 * This is the client half of `/sign-up`; the route's `page.tsx` is a Server Component that already
 ...
 * appears *after* mount — a second tab signing in, for instance — which no server-render check can
 * cover.
 *
 * @returns The account-creation screen.
 */
export function SignUpClient(): JSX.Element {
      ...
      >
        {step === 'collect' ? (
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <button
              type="button"
              className="text-on-surface-variant hover:text-on-surface text-body-small self-start underline-offset-4 hover:underline"
              onClick={() => {
                setError(null);
                setStep('collect');
              }}
