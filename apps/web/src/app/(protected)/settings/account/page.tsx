...
import { useSettings, useAccount } from '@/hooks/use-settings';
...
function getTimezoneInfo(
  ...
  try {
    ...
    // Calculate offset by comparing to UTC
...
export default function AccountSettingsPage() {
  ...
  // Update current time every minute for live timezone display
  useEffect(() => {
  ...
  // Memoize timezone info to avoid recalculating on every render
  // DST is handled automatically by the Intl API - offsets update based on current date
  ...
  // Group timezones by region and sort by offset within each region
  const groupedTimezones = useMemo(() => {
    ...
    // Sort each region by UTC offset (smallest to largest)
  ...
  const regionOrder = ['UTC', 'Americas', 'Europe', 'Africa', 'Middle East', 'Asia', 'Oceania'];
  ...
  const handleSaveName = () => {
    update({ preferredName: preferredName || null });
  ...
  const handlePlanningTimeChange = (time: string) => {
    update({ dailyPlanningTime: time || null });
  };

  const handleReviewTimeChange = (time: string) => {
    update({ dailyReviewTime: time || null });
  };

  const isLoading = settingsLoading || accountLoading || timezoneLoading;
    ...
    <div className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              />
              {hasNameChanges && (
                <Button onClick={handleSaveName} disabled={isUpdating} size="sm">
                  Save
                </Button>
              )}
            </div>
      ...
      <SettingsSection title="Preferences" description="Customize your daily planning experience.">
        <div className="divide-outline-variant divide-y">
          ...
          <SettingsRow
            ...
            description="When to remind you to plan your day"
          >
            <Input
              type="time"
          ...
          <SettingsRow
            ...
            description="When to remind you to review your day"
          >
            <Input
              type="time"
      ...
      {account && (
        <SettingsSection
          ...
          description={`Member since ${new Date(account.createdAt).toLocaleDateString()}`}
        >
