    ...
    editorProps: {
      ...
      handleKeyDown: (view, event) => {
        ...
        if (event.altKey && event.key === 'F10' && editorRef.current?.isActive('table')) {
          // The controls can live outside the editor surface so its scrollport cannot clip them.
          // Keep a direct handle because a page can mount more than one inactive editor menu.
          const firstTableControl =
            tableControlsRef.current?.querySelector<HTMLElement>('button:not(:disabled)');
    ...
    <div
      ...
      onBlur={(event) => {
        ...
        if (
          ...
          event.relatedTarget.closest('[data-table-controls], [data-editor-table-menu]') !== null
        ) {
          // Both table surfaces can live outside this wrapper so the editor scrollport cannot
          // clip them. They still belong to the same edit and autosave focus session.
          return;
        }
