/**
 * Health presentation helpers for the Projects screens.
 *
 * @remarks
 * A Project's `health` is the shared judgment verdict (`on_track | at_risk | off_track`)
 * used across Projects, Programs, and Initiatives (data-model §4; {@link Health}). These
 * helpers map the verdict onto display-ready copy and the semantic token classes used by the
 * Project health pill, so health color stays consistent across the screen and never reaches
 * for a raw hex value. Kept local to the Projects screen (rather than imported from another
 * screen's folder) so the screen owns its own presentation surface.
 */
...
/** A small solid health dot color. */
