 ...
 *
 * This file owns the **label** contract: which words each cluster shows for each real
 * `useSession()` result, driven through `useMarketingAuthState` rather than a stub, so the
 * "an errored read is not a session" rule is exercised end to end. The **destination** contract
 * moved to `tests/auth/marketing-cta.test.tsx` when it stopped depending on the session at all:
 * every primary control now points at `/open`, a Server Component that resolves the target on the
 * server. Deriving a link target from a client read was a measured defect — the CTA advertised
 * `/sign-up` for the first ~345ms of every load — so the two contracts are deliberately no longer
 * the same contract.
 */
