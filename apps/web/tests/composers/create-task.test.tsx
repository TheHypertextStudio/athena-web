...
const PARENT_TASK_ID = '01ARZ3NDEKTSV4RRFFQ69G5FAV';
...
beforeEach(() => {
  ...
  createObjectState.current = {
    ...
    closeCreate: vi.fn(),
    confirmCreate: vi.fn(),
    enqueueTask: (run: () => Promise<void>) => run(),
    openCreate: vi.fn(),
  };
...
describe('CreateTaskDialog — robust composer', () => {
  ...
  it('does not leave a blank context row in the legacy single-team composer', async () => {
    ...
    expect(document.querySelector('[data-composer-context-row]')).toBeNull();
  ...
  it('does not add a legacy context row to a template-only composer', async () => {
    templatesGet.mockResolvedValue(
      jsonResponse(true, {
        items: [taskTemplate('My template', 'personal', ADA_ID, null)],
      }),
  ...
  it('keeps contextual title, cycle, and parent task when creating an expanded draft', async () => {
    taskPost.mockResolvedValue(jsonResponse(true, { id: 'task_context', title: 'Expanded' }));
    renderComposer({
      defaultTitle: 'Expanded',
      defaultCycleId: CYCLE_ID,
      defaultParentTaskId: PARENT_TASK_ID,
    });

    expect(screen.getByLabelText<HTMLInputElement>('Task title').value).toBe('Expanded');
    fireEvent.click(screen.getByRole('button', { name: 'Create task' }));
