...
describe('personal workspace sections', () => {
  ...
  it('still surfaces the personal-preference sections (Language + Integrations)', () => {
    expect(personalKeys).toContain('vocabulary');
    expect(personalKeys).toContain('integrations');
  });

  it('exposes a Danger zone for deleting personal data', () => {
    const danger = settingsSections(true).find((s) => s.key === 'danger');
    expect(danger).toBeDefined();
    expect(danger?.status).toBe('coming-soon');
  });
