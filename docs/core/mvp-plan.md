...
Docket is free by default for one personal workspace with planning, scheduling, and time tracking.
After account creation, a personal selection creates that workspace and proceeds directly to
connection setup. Connection copy names what Docket imports and explains that imported records
remain related to their source. Passkey setup states the action and the available skip path.

Docket Pro is a separate $8-per-organization monthly product. It adds shared work, integrations,
MCP, and current Athena functionality. A 14-day trial is attached only to the organization's first
Docket Pro checkout. Future products are not described in the public product until they exist.
