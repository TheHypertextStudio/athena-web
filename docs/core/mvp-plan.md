...
- **Cycle:** a **list with a collapsible stats banner** (a **burn‑up line** — planned work done vs. remaining — plus capacity, scope changes, carryover); tasks **grouped by Project/Program**; carryover is **reviewed before it rolls** (at cycle close, keep / move / return‑to‑Triage each leftover). That "nothing rolls by accident" posture is specific to _closing_ a cycle; _filling_ one is opt-in the other direction — an **Assign backlog** action on the detail screen sweeps the team's still-unscoped, open tasks in explicitly, and a team can enable an automation rule so every new task lands straight into whichever cycle is current the moment it's created.
  A manager sets each team's cadence from 1 through 365 calendar days in the Team Settings tab.
  Changing that cadence preserves every existing assignment and starts the new schedule only after
  the last old cycle that contains planned work. Task creation, task detail, and Canvas use the
  same assignment picker. It prepares the next calendar quarter when opened and can generate the
  cycle containing any later date through **Go to date…**, so the product imposes no planning
  horizon.
...
Settings belongs to the user because Athena is the user's digital chief of staff and Docket is a centralized place to view data from many streams. It opens as a large modal over the app — a "Preferences" panel in the Notion/Claude Desktop sense, not a page you navigate to — since that maps onto the native desktop convention (open it, make a change, close it, land back exactly where you were) better than a full-page navigation would. The Personal group is ordered around the user's relationship with Athena: Profile, Athena, Connections, Connected accounts, Connected apps, Notifications, Calendar, Security, and Data & privacy. Connections are outbound: they are the apps Athena uses as data sources. Connected accounts are the external identities (e.g. a linked Google account) tied to the user's own Docket identity. Connected apps are inbound: external clients the user has authorized to access Docket. Workspace administration is reached from a workspace-switcher chip inside the same modal, not a separate "Workspaces" destination — switching workspaces there is a real navigation to that workspace's settings (General, Members & Access, work configuration, and, for a manager, Publishing), so it never leaves the Personal group behind.
...
- One **14-day, card-required Docket Pro trial** per eligible organization. Personal baseline features remain writable when Pro ends. Shared work becomes read-only after the paid period or the seven-day payment-recovery deadline. Administrators can export or reactivate at any time, and ending Pro never schedules data deletion.
...
This screen is the only one in the plan that customers never see. It is the back office our team uses to answer support questions, reconcile billing, and monitor service health without changing database columns by hand. It is organized around people because one person may run several Docket organizations. Support and finance staff can inspect provider state, retry safe synchronization, extend an eligible Stripe trial, preview and issue credits, and review discount applications according to their roles. Every action requires a reason and enters the audit log. Subscription cancellation and payment failure change Pro access only. They never start account deletion. A confirmed account deletion through the customer's Danger Zone remains the only product action that starts the separate deletion process. Support staff may temporarily view the product as a customer only through the warning-wrapped, time-limited, reason-logged, and audited flow. The agent-oversight view shows aggregate health signals and never exposes session contents.
...
- **Safe billing actions inline** (reconcile Stripe, extend an eligible Stripe trial, preview or issue a credit, and review discounts), with provider-confirmed results and a full audit trail.
- A **billing-access view** for trial, active, grace, cancellation, read-only, discount, and complimentary states. Account deletion remains a separate confirmed Danger Zone flow.
