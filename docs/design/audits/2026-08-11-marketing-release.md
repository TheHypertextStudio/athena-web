...
The product frames were regenerated on 2026-08-12 from a freshly migrated isolated PGlite
database through real passkey sign-up, application API writes, and MCP OAuth consent. The example
account is Alex Rivera; its personal planning, Civic Studio, and Neighborhood Fund records use
specific descriptions, dates, estimates, schedules, time entries, subtasks, and an attachment.
...
| 8. Detail craft                   |     3 | Borders, crops, and disclosure badges remain aligned at both widths. Every route passed the automated 320px overflow check after the sub-360px header hid its redundant sign-in link; all nine screenshot assets resolve with literal alt text and no placeholder frame remains. |
