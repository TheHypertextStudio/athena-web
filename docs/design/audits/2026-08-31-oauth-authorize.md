...
| 5. Color discipline               | 3     | The scroll masks use `bg-gradient-to-{b,t} from-surface to-transparent` — the card's own plain tone, not a hardcoded value. Once rows moved to individually tonal, gap-separated chips (finding 3), what actually shows behind the mask is the card background visible in those gaps, not the old shared `surface-container-high` block — so the mask token tracks that. Confirmed matching in both themes (`consent-expanded-1440x900-{light,dark}.png`) — the fade reads as the card itself thinning out, not a foreign overlay. |
...
1. **The capped scope list silently clipped content with no indication more existed.** —
   ...
   `surface → transparent` gradient at whichever edge does. Before/after:
