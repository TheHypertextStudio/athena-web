...
Screenshots: `screenshots/2026-09-08-create-composers/` contains 38 captures. Task, Project,
...
overflow probe. Project has Description and Milestones captures at 1440×900, 390×844, and 320×844
in both themes.
...
| 3. Spatial rhythm         | 3     | At 390×844 and 320×844, the shell fills the dynamic viewport and gives the selected body section the remaining height between its fixed header and footer. The Project description surface never falls below two-thirds of that body. The title-to-body boundary uses one 12px phone step instead of four stacked steps. The body uses the shared responsive inline inset, which preserves safe-area sides without adding block padding. The centered desktop shell retains its 42rem compact and 64rem expanded measures plus its 60dvh/36rem and 80dvh/48rem height rules. |
| 4. Hierarchy              | 3     | The five-second path is title, optional summary, section chips, selected body, properties, then create. Project uses compact filled `Description` and `Milestones n` chips directly below its summary, matching the main detail page's information order without adding a second resting surface. Metadata has its own first footer row. `Create more` and the primary submit occupy the second row.                                                                                                                                                                         |
...
| 7. States completeness    | 3     | The captures cover the initial empty state, disabled submission, compact metadata overflow, expanded editing, both themes, and minimum-width layout. Project adds populated Description and Milestones states with a live count. The browser check switches sections twice and confirms that the TipTap node, description text, effective scroll position, and milestone draft all survive. The template action appears below the description hint where templates are enabled.                                                                                              |
...
6. **The title-to-editor boundary stacked four independent spacing rules.** The title block,
   responsive header, responsive body, and editor each contributed 12px, which produced the visible
   48px gap. The body now uses the shared inline-only responsive inset, and the old title and editor
   margins are gone. The browser matrix rejects a rendered boundary gap above 16px.
7. **Project milestones no longer compete with the writing surface.** The old stacked milestone
   fields could squeeze the description into a short strip. Project now uses filled header chips to
   switch one full-height body between Description and Milestones. Both panels stay mounted, while
   the inactive panel uses `aria-hidden` and `inert`; this keeps local drafts and scroll state without
   exposing duplicate controls to assistive technology. The browser matrix rejects any visible
   description surface shorter than two-thirds of the shared body.
