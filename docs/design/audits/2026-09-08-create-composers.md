...
copy. This review does not claim a browser screenshot for a route that does not exist.
...
| 3. Spatial rhythm         | 3     | The compact shell measures 42rem, or 672px at the captured root size. The expanded shell measures 64rem, or 1024px. Both keep the existing 60dvh/36rem and 80dvh/48rem height rules. The editor, metadata row, and action row share one inset.                                                                                  |
...
| 6. Motion & feedback      | 3     | The Radix dialog retains its shared open, close, focus, and reduced-motion behavior. Expand and collapse use separate Maximize and Minimize glyphs and keep the same editor node mounted. The production browser check confirms that editor scrolling does not move the title or footer.                                        |
...
| 8. Detail craft           | 3     | The shared close and expand controls use the same 28px desktop geometry and separate 40px coarse-pointer squares with a 4px gap. The header reserves both control columns. The 320px probe reports no page overflow. The editor owns vertical scrolling and contains wheel overscroll, while the title and footer remain fixed. |
