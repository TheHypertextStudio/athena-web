...
with a task row selected beside both panels (`row-selected-light`), at 1024×768 with the inspector
...
| 6. Motion & feedback              | 3     | Columns enter with the shared fade-and-slide and stop under reduced motion; the frame moves only when Athena adds nodes, when the inspector docks over a board wider than the strip it leaves (a 450ms refit, `row-selected-light`), or to nudge a selection out from under a panel. Handles take the accent on hover; the project header steps up a tone under the pointer.                                                    |
...
4. At 1280 both floating panels left a 530px strip for a 712px board, and a selected row pushed the
   ...
   nudging one node (`boardOverflows` in `plan-canvas-panel.tsx`; `row-selected-light` shows the
   refit at 1440).
