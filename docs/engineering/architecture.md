...
> **Current MCP note (2026-08-15).** Any historical client, vendor-domain, or CIMD-host allowlist
> below is superseded. Current MCP trust comes from OAuth registration and consent, token scopes
> and audience, organization grants, Docket Pro capabilities, and protocol-level network safety.
> See [`specs/mcp-surface.md`](./specs/mcp-surface.md).
...
The layer boundaries fall where they do because each owns exactly one concern and depends only downward, producing the strict build spine `env → db → auth → api`. `@docket/db` is the sole owner of all SQL — including the tables that back Better Auth — so there is one schema source of truth and one migration story (`drizzle-kit` against the unpooled Neon endpoint, while runtime traffic uses the pooled PgBouncer endpoint). `@docket/auth` wraps the single `betterAuth()` instance and its Drizzle adapter. `apps/api` composes the Hono router and exports its type as the `AppType` contract — and critically, that export is _type-only_: the Next apps get full RPC type inference at compile time while actual requests still travel same-origin over the rewrite at runtime. the retired contract package holds the Zod schemas that bind all three consumers (RPC handlers, MCP tools, Next server actions) to one shape. The compiled boundary is a deliberate ergonomic tradeoff in service of this design, and it is narrower than it looks: only `apps/api`'s `@docket/api/rpc-contract` subpath is compiled (`tsc` emits its `.d.ts` ahead of time) so Hono's RPC type inference doesn't overwhelm `tsserver`; every other package — including `@docket/db` and `@docket/auth` — is consumed as raw TS via `transpilePackages`, the same as the retired contract package, `@docket/ui`, and `@docket/env`. The failure modes this guards against are concrete — break the `.route()` method chain (or convert a router to `OpenAPIHono`) and the `AppType` contract silently degrades; let any consumer drift from the API's pinned Hono version and RPC inference falls apart — so these are not stylistic preferences but load-bearing constraints on the whole client-server type bridge.
...
║   the retired contract package: Zod schemas + branded Id · @docket/ui: shared components       ║
║   Runtime calls go same-origin over Vercel rewrites (/api/* → apps/api).        ║
║                                                                                  ║
║   ┌─ COMPILED (tsc → dist; types only) ─┐   ┌─ JIT (raw TS · transpilePackages) ─┐ ║
║   │  apps/api/rpc-contract               │   │  every other package (db, auth, …) │ ║
...
keeping auth cookies first-party). Only `apps/api`'s `@docket/api/rpc-contract` subpath
is **compiled** to `dist` so RPC type inference doesn't cripple `tsserver`;
every other package — including `@docket/db` and `@docket/auth`, alongside
`@docket/ui`, the retired contract package, and `@docket/env` — stays **JIT** (raw TS via
`transpilePackages`). `@docket/db` is the single owner of all SQL — including the
Better Auth-backed tables owned there — and `tooling/` holds shared tsconfig,
...
`Program` earns its own entity because each of the three containers promises something different about the work inside it. `Project` is a _bounded_ effort tracked to completion, and `Initiative` is a pure _theme_ that holds work only by association (many-to-many overlays to projects and programs). An Initiative's health is its own judgment, writable by a person, an agent, or a health-bearing update; connected-work health is derived separately. Continuous operations — a support function, a nonprofit's after-school program — are the third case: an ongoing area of responsibility that keeps running after each deliverable inside it lands. That is what a Program names, and it is the ongoing counterpart to the bounded Project. The containment graph carries the same idea: a `Project` may sit directly under the org or under a `Program` via the nullable `program_id`, and a `Task` may hang off a `Program` directly (`task.program_id`) for recurring work that belongs to no bounded project — which is why both of those parent FKs are `SET NULL` associations rather than cascades.

**Superseded:** this paragraph previously described three mutually exclusive status enums — `project_status`, `initiative_status`, and a `program_status` of `active | paused | archived` with **deliberately no `completed`** — and argued that a Program _cannot_ be marked done, which was "the whole point of the concept." Statuses are now workspace-defined rows in `work_status` (see [`specs/statuses.md`](./specs/statuses.md)), and a Program can complete: its seeded set is `Proposed | Active | Paused | Completed | Archived`, where `Archived` records a Program retired and kept for history. The distinction above is the durable one and it survives intact — it lives in what each container means and in how each is read, a Project through weighted progress toward its outcome and a Program through health and flow. What changes is that the schema stops forging the distinction by withholding a value. A Program usually runs on, and some of them genuinely end; a workspace that wants Programs that never end deletes `Completed` from its Program set, which is an opinion it now states rather than inherits.

Initiative hierarchy is a context-owned association rather than a self-parent column. An
`initiative_hierarchy_link` names the workspace context plus its parent and child Initiative. A
child has at most one parent per context, may appear in other contexts, and may belong to another
workspace. The context's configurable `initiative_max_depth` limits total levels. Reads and writes
re-authorize both endpoint workspaces; the edge never becomes a sharing grant, so viewers without
access to a referenced workspace learn nothing from the link or its rollups.
...
──────────────────────────────────────────────────────────────────────
  ...
  /internal/*  ── machine edges · self-authed (NOT session-gated):
                  ...
                  github} (provider sig) · /cron/* (CRON_SECRET)
                  · /integrations/github/callback (signed state)
...
│ Linear             socialProviders.linear — account-LINKING, not a new account│
...
The transport is a single Streamable-HTTP endpoint per the 2025-11-25 spec, handling `POST` for JSON-RPC and `GET` for the server-to-client stream. We use the official `@modelcontextprotocol/sdk` `StreamableHTTPServerTransport` rather than hand-rolling the protocol, and the deprecated two-endpoint HTTP+SSE transport is forbidden. The shipped transport is stateless: each request gets a fresh server and transport, with no `Mcp-Session-Id` or client registry. When an `Origin` header is present, protocol-level validation rejects malformed and non-public web origins to prevent DNS rebinding; native clients may omit it. This is a network safety rule, not a client approval list.

Audience binding (RFC 8707) and the no-token-passthrough rule keep the multi-tenant resource server from becoming a confused deputy. Every token must carry `aud` equal to the configured MCP resource URI, and a mismatch is a hard 401. The resource server never accepts provider tokens and never forwards the inbound client token to GitHub, Drive, Linear, or another connector. Connector calls resolve the organization's own encrypted integration credentials. URL-form client registration is handled by the CIMD validation shim: HTTPS metadata must match its client identifier, remain within strict response bounds, and resolve only to public destinations. Any client that satisfies those rules may proceed to user consent; vendor identity is not an authorization input.

The tool/resource split maps cleanly onto the read/write distinction. **Tools are mutations** — `create_task`, `move_task`, `set_task_assignee`, `post_update`, `link_external`, `trigger_agent_session`, `approve_action`, and the rest — each authored with Zod input _and_ output schemas in the retired contract package, converted to JSON Schema 2020-12, and carrying explicit `ToolAnnotations` (`readOnlyHint`/`destructiveHint`/`idempotentHint`/`openWorldHint`) rather than relying on the SDK's defaults, which default `destructiveHint` to true. Closed-world tools touching only Docket's own DB set `openWorldHint: false`; the two that reach external systems — `link_external` and `trigger_agent_session` — set it true. **Resources are addressable-by-URI reads** under the `docket://{org}/{type}/{id}` scheme, returning Zod-validated read DTOs as JSON. The one nuance worth internalizing is that two _read_ operations, `run_view` and `search`, are exposed as tools rather than resources precisely because they take rich query arguments that don't fit a URI; they keep `readOnlyHint: true` and need only `work:read`. A subtle ID-primitive detail from RECONCILIATION applies here: although the spec's draft schemas use `z.string().uuid()`, the canonical decision is ULID `text` IDs repo-wide, so MCP tool schemas must consume the shared branded `Id` from the retired contract package and _not_ assert UUID shape — the addressing differs (`docket://{slug}` vs REST `/v1/orgs/:orgId`) but the id primitive is identical.
