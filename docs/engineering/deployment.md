...
| `PASSKEY_RP_ID`             | bootstrap/manual     | WebAuthn relying-party domain, and reused as `BETTER_AUTH_COOKIE_DOMAIN` (deploy.yml) so the session cookie is valid across all three `*.hypertext.studio` hosts, not just whichever one issued it — required for the MCP OAuth authorize flow, which sends the browser directly to `docket-api.hypertext.studio` rather than through the web app's same-origin `/api/auth` rewrite. Use `hypertext.studio` for the production `*.hypertext.studio` hosts. |
...
`NODE_ENV`, `APP_MODE`, `API_URL`, `WEB_URL`, `BETTER_AUTH_URL`, `BETTER_AUTH_TRUSTED_ORIGINS`, `BETTER_AUTH_ALLOWED_HOSTS`, `BETTER_AUTH_COOKIE_DOMAIN`, `BETTER_AUTH_PASSKEY_RP_ID`, `BETTER_AUTH_PASSKEY_RP_NAME`, `GOOGLE_CALENDAR_WEBHOOK_URL`, `GOOGLE_OAUTH_PUBLIC`, `GOOGLE_OAUTH_TEST_EMAILS`, `BILLING_ENABLED`, `MCP_ALLOWED_ORIGINS`, `MCP_TASKS_ENABLED`, `MCP_CIMD_STRICT`.
