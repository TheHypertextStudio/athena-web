...
| `athena-triggers`                      | Run every due user-owned scheduled Athena trigger (five-minute minimum schedule), then drain agent-assigned tasks out to the delegation surface and post finished work back as gated proposals | every 1 min              |
