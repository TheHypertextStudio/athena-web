...
6. **Capability wire form:** the bare literal (`view|comment|contribute|assign|manage`) is the stored/Zod value (`Capability` in the retired contract package). `org:*` is **only** an API route-annotation label.
7. **Shared `Id` schema:** the retired contract package exports one branded `Id` (ULID-shaped) consumed by **both** the API and MCP tool schemas. MCP must **not** use `z.string().uuid()`. Addressing differs intentionally — REST `/orgs/{id}`, MCP `docket://{slug}` — but the id primitive is identical.
