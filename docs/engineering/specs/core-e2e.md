...
| `e2e/scheduling/task-calendar-scheduling.spec.ts`        | Yes  | A user schedules a task from its action menu and the real API persists a 30-minute calendar block with a contained task link.                                                                                                       |
| `e2e/scheduling/fluid-scheduling-relations.spec.ts`      | Yes  | A user drags an event onto another event and into a time block to create related and contained calendar links.                                                                                                                      |
