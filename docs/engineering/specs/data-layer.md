...
| 5     | staleTime tiers; API pagination + aggregate detail endpoints                             | 🚧 **staleTime tiers complete** — volatile (task state, sessions, counts), static (members/roles/invitations, teams, org vocabulary, connected apps, identities), standard (lists); the rest inherit the 30s default by design. **API pagination + aggregate detail endpoints deferred** — a separate full-stack effort that (a) reimplements the project-detail composition (10 reads + an N-task / M-initiative / per-session waterfall, see `lib/fetch-project-detail.ts`) and the cycle stats fan-out server-side, which cannot be runtime-verified without a DB/API server, and (b) modifies `apps/api/.../tasks.ts` + `packages/types/src/task.ts`, both carrying live uncommitted edits on the integration branch. Gated on coordinating with that branch to avoid API drift |
