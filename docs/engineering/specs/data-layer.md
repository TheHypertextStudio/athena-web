...
| 4     | SSR prefetch + hydration on entry/detail pages                                           | 🚧 foundation + **projects page** SSR'd (proven pattern: server `page.tsx` prefetches → `<HydrationBoundary>` → client). Remaining entry pages follow the same shape — land them `next build`-verified to avoid compounding unverified-RSC risk (§7)           |
