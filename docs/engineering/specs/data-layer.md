...
7. **Lists are bounded/paginated; detail pages use aggregate endpoints** — no N+1 waterfalls. _(Phase 5 — shipped: keyset cursor pagination on every list endpoint via `lib/list-cursor` + `CursorQuery`; aggregate `projects/:id/rollup` + inline cycle-list stats collapse the detail/list waterfalls. Client `useInfiniteQuery` is adopted per-surface when a list needs incremental rendering.)_
...
| 5     | staleTime tiers; API pagination + aggregate detail endpoints                             | 🚧 **staleTime tiers complete** — volatile (task state, sessions, counts), static (members/roles/invitations, teams, org vocabulary, connected apps, identities), standard (lists); the rest inherit the 30s default by design. **Aggregate / N+1 collapses complete — both verified against the PGlite route-test harness.** ✅ Cycles list returns each cycle's stats inline (one batched `inArray` query via `committedTasksForCycles` → the same pure `computeStats`), collapsing `fetch-cycles-with-stats` from 1+N round-trips to one. ✅ Project detail: new `GET /projects/:id/rollup` (a small sibling router so it lands in clean files) returns the task→milestone map (`task.milestone_id`) + the project's initiative (inverse `initiative_project` lookup) in one read, killing `fetch-project-detail`'s per-task N+1 and per-initiative M+1. ✅ **Keyset cursor pagination on every org list endpoint** — cycles, programs, initiatives, **tasks, and projects** — verified against the PGlite harness (a 5-endpoint page-walk test). A universal seek helper (`lib/list-cursor`) + a backward-compatible `CursorQuery` (optional `limit`): omitting `limit` returns the full list as before (no caller truncated, SSR hydration unchanged), supplying it returns a bounded newest-first page (`createdAt`/`startsAt`, id tiebreak) + `nextCursor`. The tasks/projects edits were committed surgically (index injection) to keep the integration branch's in-flight `tasks.ts`/`projects.ts` WIP untouched. **Remaining:** only the client `useInfiniteQuery` adoption the plan scopes to genuinely large lists (tasks, triage) — the server mechanism is ready, but today's task surfaces (my-work, triage, project-detail) load-and-filter and are SSR-hydrated, so adopting incremental loading now would regress that hydration for no present benefit; adopt per-surface when a list actually needs incremental rendering |
