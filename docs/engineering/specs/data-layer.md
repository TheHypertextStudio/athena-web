...
| 5     | staleTime tiers; API pagination + aggregate detail endpoints                             | 🚧 **staleTime tiers complete** — volatile (task state, sessions, counts), static (members/roles/invitations, teams, org vocabulary, connected apps, identities), standard (lists); the rest inherit the 30s default by design. **Aggregate / N+1 collapses complete — both verified against the PGlite route-test harness.** ✅ Cycles list returns each cycle's stats inline (one batched `inArray` query via `committedTasksForCycles` → the same pure `computeStats`), collapsing `fetch-cycles-with-stats` from 1+N round-trips to one. ✅ Project detail: new `GET /projects/:id/rollup` (a small sibling router so it lands in clean files) returns the task→milestone map (`task.milestone_id`) + the project's initiative (inverse `initiative_project` lookup) in one read, killing `fetch-project-detail`'s per-task N+1 and per-initiative M+1. **Remaining: cursor pagination** — the plan scopes `useInfiniteQuery` to genuinely large lists (tasks, triage), both in the task-domain files under live WIP (`tasks.ts`, `task-helpers.ts`); the other lists are small enough that pagination is low-value. Deferred to coordinate with the integration branch and avoid API drift |
