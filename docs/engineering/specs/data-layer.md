...
| 5     | staleTime tiers; API pagination + aggregate detail endpoints                             | 🚧 **staleTime tiers complete** — volatile (task state, sessions, counts), static (members/roles/invitations, teams, org vocabulary, connected apps, identities), standard (lists); the rest inherit the 30s default by design. **Aggregate / N+1 work — in progress, verified against the PGlite route-test harness.** ✅ Cycles list now returns each cycle's stats inline (one batched `inArray` task query via `committedTasksForCycles` → same pure `computeStats`), collapsing the web `fetch-cycles-with-stats` roster from 1+N round-trips to one. Remaining: the **project-detail** aggregate (`lib/fetch-project-detail.ts`'s N-task milestone lookup + M-initiative timeline loop + per-session fan-out) — the marquee milestone-per-task fix is entangled with the in-flight `tasks.ts`/`TaskOut` edits; and **list pagination**, low-value on today's small lists and blocked on the tasks endpoint (live WIP). Gated slices coordinate with the integration branch to avoid API drift |
