...
| 4     | SSR prefetch + hydration on entry/detail pages                                           | 🚧 foundation + **6 pages SSR'd**: projects, programs, teams, portfolio, inbox, initiatives (server `page.tsx` prefetches → `<HydrationBoundary>` → client). Remaining: **cycles** (composite + a server teams→teamIds step), **my-work** (rides hand-rolled `use-my-work` — migrate to the query layer first), **today** (WIP). All typecheck-only here — land `next build`-verified (§7) |
