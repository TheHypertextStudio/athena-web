...
7. **Lists are bounded/paginated; detail pages use aggregate endpoints** — no N+1 waterfalls. _(Phase 5 — shipped: keyset cursor pagination on every list endpoint (`lib/list-cursor` + `CursorQuery`); `projects/:id/rollup` and the Initiative overview/detail aggregates collapse their respective detail and hierarchy waterfalls; inline cycle-list stats collapse the cycle list waterfall; cycle windows auto-roll in-process (no `/current` fan-out). Initiative aggregate keys include the viewing workspace context because hierarchy visibility and inherited rollups are viewer- and context-specific. **Pagination policy:** the cursor is a server-side **safety bound**. The filtered rosters (projects / cycles / programs / initiatives / my-work) deliberately fetch the full set — they filter & group client-side over all rows via `applyView`, so paginating them would truncate the filter input or add round-trips for the same data. Client `useInfiniteQuery` (`apiInfiniteQueryOptions` / `useApiInfiniteQuery`) is reserved for genuinely-large **append feeds** that render incrementally — the activity / Stream firehose — not the filtered rosters.)_
...
Initiative entry and detail pages prefetch the aggregate definition for the active workspace context.
