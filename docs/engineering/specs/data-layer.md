...
| 4     | SSR prefetch + hydration on entry/detail pages                                           | 🚧 foundation + **projects, programs, teams** list pages SSR'd (server `page.tsx` prefetches → `<HydrationBoundary>` → client). Remaining: composite-fetcher pages (cycles/initiatives need a server-api fetcher), Hub pages (portfolio/inbox/my-work), today (WIP). Land `next build`-verified (§7) |
