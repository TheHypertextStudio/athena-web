...
| 5     | staleTime tiers; API pagination + aggregate detail endpoints                             | 🚧 **staleTime tiers complete** — volatile (task state, sessions, counts), static (members/roles/invitations, teams, org vocabulary, connected apps, identities), standard (lists); the rest inherit the 30s default by design. **Aggregate / N+1 collapses complete — both verified against the PGlite route-test harness.** ✅ Cycles list returns each cycle's stats inline (one batched `inArray` query via `committedTasksForCycles` → the same pure `computeStats`), collapsing `fetch-cycles-with-stats` from 1+N round-trips to one. ✅ Project detail: new `GET /projects/:id/rollup` (a small sibling router so it lands in clean files) returns the task→milestone map (`task.milestone_id`) + the project's initiative (inverse `initiative_project` lookup) in one read, killing `fetch-project-detail`'s per-task N+1 and per-initiative M+1. ✅ **Keyset cursor pagination** on the org list endpoints, verified against the PGlite harness: a universal seek helper (`lib/list-cursor`) + a backward-compatible `CursorQuery` (optional `limit`) wired into **cycles, programs, initiatives** — omitting `limit` returns the full list as before (no caller truncated, SSR hydration unchanged), supplying it returns a bounded newest-first page + `nextCursor`. **Remaining (blocked on live WIP):** the **tasks** and **projects** list endpoints (`tasks.ts`/`projects.ts` carry uncommitted integration-branch edits) and the client `useInfiniteQuery` adoption the plan scopes to the large lists (tasks, triage) — they adopt the same `list-cursor` helper once those routes settle. Deferred only to avoid API drift on files under active edit |
