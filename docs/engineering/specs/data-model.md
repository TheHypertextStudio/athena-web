...
Billing does not add a Better Auth table. Better Auth owns identity and sessions. The Docket billing
schema owns `organization_billing_account`, `billing_checkout_attempt`,
`billing_provider_event`, `billing_discount_program`, `billing_discount_application`,
`billing_discount_application_event`, `billing_discount_evidence`, `billing_discount_award`,
`billing_provider_sync`, `billing_credit`, and `organization_product_entitlement`. The admin schema
owns audited `billing_exemption` grants for complimentary access. The current field and transition
contract lives in [`product-billing.md`](./product-billing.md).
...
- Personal space = `organization{ is_personal: true }` with one default team. Organization billing
  uses `organization_billing_account.organization_id`, not a Better Auth subscription reference.
