...
| `BETTER_AUTH_PASSKEY_RP_ID`                         | api (auth) | server | D=P    | WebAuthn Relying Party ID. Dev: `localhost`. Prod: the shared apex (`hypertext.studio`). Must be a registrable suffix of all app origins.                                                                                                 | Bootstrap: `localhost` for dev; prompts for apex in prod.                                 |
...
For a non-local RP ID, the auth server also derives `https://<BETTER_AUTH_PASSKEY_RP_ID>` and adds
that origin to the passkey verifier and request-origin allowlist. Apple Authentication Services
uses that associated-domain origin for native passkey ceremonies. Localhost keeps Better Auth's
...
| `APPLE_CLIENT_ID`           | api (auth) | server | P only | Sign in with Apple **Services ID** (e.g. `com.docket.web`) — the OAuth `client_id` + JWT `sub`. Web-only; **all four `APPLE_*` or none**. Apple rejects `localhost`, so it does not run in local dev. | Apple Developer → Certificates, IDs & Profiles → **Identifiers → Services IDs**.            |
| `APPLE_APP_CLIENT_ID`       | api (auth) | server | P only | Native Apple App ID bundle identifier accepted as a second ID-token audience. It must be the primary App ID associated with `APPLE_CLIENT_ID`.                                                        | Apple Developer → Certificates, IDs & Profiles → **Identifiers → App IDs**.                 |
| `APPLE_TEAM_ID`             | api (auth) | server | P only | Apple 10-char Team ID — the client-secret JWT `iss`.                                                                                                                                                  | Apple Developer → **Membership** → Team ID.                                                 |
| `APPLE_KEY_ID`              | api (auth) | server | P only | The `.p8` Sign-in key's Key ID — the JWT header `kid`.                                                                                                                                                | Apple Developer → **Keys** → your "Sign in with Apple" key → Key ID.                        |
| `APPLE_PRIVATE_KEY`         | api (auth) | server | P only | The downloaded `.p8` PKCS#8 PEM. **There is no `APPLE_CLIENT_SECRET`** — the API mints a 180-day ES256 JWT from this at boot (`generateAppleClientSecret`), so it never silently expires.             | Same **Keys** entry → download the `.p8` once (multiline; store verbatim or `\n`-escaped).  |

Apple follows the social-provider callback route (`${BETTER_AUTH_URL}/api/auth/callback/apple`), and
`buildAuthOptions` auto-adds `https://appleid.apple.com` to `trustedOrigins` when it is configured
(Apple posts the callback form_post from there). Because Apple rejects non-HTTPS/`localhost`, it is
`P only` — exercise it on a prod/preview HTTPS origin (or the shared tunnel), never plain `.localhost`.
