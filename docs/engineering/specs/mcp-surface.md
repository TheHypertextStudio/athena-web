...
- **Task visibility for agents:** a registered agent sees only the Tasks its own grants cover, public or private. A workspace grant covers every Task in the workspace, and `find`, `list_work`, `update`, and `archive` also require it, because they first check `view` on the workspace. A project, program, team, or task grant alone lets the agent read the Tasks it covers by id with `get_tasks`.
