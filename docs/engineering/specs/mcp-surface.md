...
- Internal Athena execution uses an in-process agent principal and does not traverse the public OAuth resource. The Docket web session cookie is not accepted by `/mcp`.

> **RESOLVED:** Better Auth 1.6.14 does NOT validate URL-form `client_id`s — its authorize handler resolves clients by exact `client_id` lookup only. The thin CIMD shim exists (`apps/api/src/mcp/cimd.ts`: fetch + validate + SSRF guard + upsert into `oauth_application`) and `cimdAuthorizeMiddleware` is mounted ahead of `/api/auth/mcp/authorize` in `apps/api/src/server.ts`.
>
...
8. Env contract (validated in `@docket/env`, dev mirrors prod): **DONE, on-by-default.** `MCP_ISSUER_URL`/`MCP_RESOURCE_URL`/`OIDC_LOGIN_PAGE_URL` derive automatically from the required `API_URL`/`WEB_URL` (`packages/env/src/api.ts`) — no client/vendor setup is needed. If Tasks are enabled, `MCP_SESSION_STORE_URL` and `MCP_TASKS_ENABLED` remain explicit.
