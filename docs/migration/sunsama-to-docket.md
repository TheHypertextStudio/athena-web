...
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `id`                   | `task.externalId` (with `task.source = "linked"`, `task.sourceIntegrationId` = a real `migration`-pattern integration row) | The migration join key. Unique per (integration, externalId), so a re-run **updates rather than duplicating — proven, not just intended**: `sunsama-connector.ts` stamps every task's Sunsama id onto `ImportedItem.provenance.externalId`, and a real `--apply` run followed by a second one against the unchanged fixture inserts all 10 rows (7 tasks + 3 subtask child rows) the first time and 0 the second (see §5.2).    |
...
| `plannedDate`          | `task.startDate`                                                                                                           | Sunsama's "the day I intend to do this" is Docket's **start** date, not its due date — conflating the two would invent deadlines that were never set. Written by `--apply` since the §5.3 contract extension (`ImportedItem.startDate`); the applied run report's `persistedByReconciler.startDate` counts the rows that actually carry it.                                                                                     |
| `dueDate`              | `task.dueDate`                                                                                                             | Direct, and written by `--apply` (`ImportedItem.dueDate` is on the shared write path).                                                                                                                                                                                                                                                                                                                                          |
| `timeEstimateMinutes`  | `task.estimateMinutes`                                                                                                     | Direct; both are minutes. Written by `--apply` since the §5.3 contract extension (`ImportedItem.estimateMinutes`); counted per run in `persistedByReconciler.estimateMinutes`.                                                                                                                                                                                                                                                  |
...
| `subtasks`             | child task rows (`task.parentTaskId`)                                                                                      | Each subtask becomes its own Docket task parented to the migrated one, so its title survives as work rather than as prose in a note. Written by `--apply` since the §5.3 contract extension: each subtask rides as its own `ImportedItem` with `parentExternalId` pointing at the parent, keyed by Sunsama's subtask id (or a stable `<parent>/subtask-<n>` when Sunsama supplied none) so a re-run matches the same child row. |
| `subtasks[].completed` | child `task.state`                                                                                                         | Each subtask keeps its own completion, mapped to the destination team's workflow states exactly as the parent's is.                                                                                                                                                                                                                                                                                                             |
...
dedicated database, produced [`sunsama-run.json`](./sunsama-run.json): all 10 rows created — the 7
active fixture tasks plus their 3 subtasks as child rows — `docketMatchedCount: 10`, both unmatched
lists `[]`. Run a **second** time against the same database, every row's
`sourceIntegrationId` + `externalId` already matches a `linked` row, so `reconcileTasks` reports
`inserted: 0` for all four workspaces (`alreadyPresent` covers all 10 instead) — zero duplicates,
zero mutated pre-existing rows. `apps/api/tests/routes/integration-reconcile-sunsama.test.ts`
asserts exactly this (read → map → route → reconcile → a second reconcile is a no-op) against the
pglite test harness, so it runs in every `pnpm test` without a database of its own.
...
### 5.3 CLOSED: the shared write path persists the planned day, the estimate, and subtasks

This section used to name the one field gap left after §5.2: `ImportedItem` — the port every
connector/migration adapter maps into — carried `title`/`body`/`completed`/`dueDate` and
provenance but no `startDate`, no `estimateMinutes`, and no parent-task linkage, so
`reconcileTasks` dropped all three for **every** adapter and the run report listed the per-task
losses under `notWrittenByReconciler`.

It is closed by extending the shared contract itself, connector-generically (nothing in it is a
Sunsama-ism):

- **`ImportedItem.startDate` / `ImportedItem.estimateMinutes`** — written by `insertLinked` on
  insert and by `applyPull` on pull. `undefined` means the provider has no such concept, and a
  pull leaves the local value alone — so a connector that never sets them (Google Tasks, GitHub,
  Linear, Notion today) cannot clear a value a user set in Docket. An explicit `null` means "the
  source says unset" and clears on pull. Zero minutes is a real estimate, not "unset".
- **`ImportedItem.parentExternalId`** — makes the item a child task. `reconcileTasks` orders each
  batch parents-first (children sort after their parents by depth in the batch's parent chain), so
  a child inserted in the same run as its parent still gets a real `task.parentTaskId`. A parent
  id that resolves to nothing degrades to a top-level insert instead of failing the run —
  hierarchy is metadata, not a precondition for keeping the work.
- **Sunsama's side**: each subtask now rides as its own `ImportedItem` (see
  `SunsamaImportedTask.childItems`), keyed by Sunsama's subtask id or a stable
  `<parent>/subtask-<n>` when Sunsama supplied none, carrying its own completion state, so a
  re-run reconciles children as idempotently as parents.

`notWrittenByReconciler` is gone from the connector and the run report — there is nothing left it
could honestly contain. In its place an applied run reports `persistedByReconciler`, counted from
the **database** after the reconcile rather than echoed from the input: on the fixture,
`startDate` on 3 rows (su-004/su-005/su-006 — the three with a planned day), `estimateMinutes` on
5 rows, and 3 subtask child rows. The second `--apply` run against the same database reports the
same counts with `created: 0` and `alreadyPresent: 10` — the closure is idempotent, not just
one-shot.
