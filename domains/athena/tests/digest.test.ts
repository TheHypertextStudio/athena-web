...
import {
  ...
  parseHighlights,
  reconcileHighlights,
  type NarrateDayInput,
  type NarrationEpisode,
} from '../src/digest';
...
describe('buildRequest', () => {
  ...
  it('explicitly requires first-person narration rather than third-person recipient prose', () => {
    const req = buildRequest(INPUT, CONFIG);

    expect(req.system).toContain('"I merged');
    expect(req.system).toContain('not "Willie merged');
  });
...
describe('parseHighlights', () => {
  ...
  it('returns nothing for a reply it cannot read', () => {
    ...
    expect(parseHighlights('null').size).toBe(0);
