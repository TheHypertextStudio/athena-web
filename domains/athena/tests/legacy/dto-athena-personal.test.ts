...
describe('personal Athena API contracts', () => {
  it('defines invocation context without accepting an owner id', () => {
    expect(
      AthenaSessionCreateBody.safeParse({
    ...
    expect(
      AthenaSessionCreateBody.safeParse({ prompt: 'Prepare it', ownerUserId: 'user_other' })
        .success,
    ).toBe(false);
  });

  it('groups personal work into product queue states', () => {
    expect(
      AthenaOverviewOut.safeParse({
  ...
  it('requires application-owned display metadata on personal work summaries', () => {
    const summary = {
      id: ID,
      kind: 'job',
      status: 'running',
    ...
    expect(AthenaSessionSummaryOut.safeParse(summary).success).toBe(true);
    expect(
      AthenaSessionSummaryOut.safeParse({
        ...summary,
        context: { workspaceId: ID, source: { type: 'project', id: ID } },
      }).success,
    ).toBe(false);
    expect(AthenaSessionSummaryOut.safeParse({ ...summary, workspace: undefined }).success).toBe(
      false,
    );
  });

  it('defines a compact pulse that cannot carry personal session history', () => {
    expect(AthenaPulseOut.safeParse({ needsYou: 2, working: 3 }).success).toBe(true);
    expect(
      AthenaPulseOut.safeParse({
        needsYou: 2,
        working: 3,
        sessions: [{ id: 'private-history' }],
      }).success,
    ).toBe(false);
  });
});
