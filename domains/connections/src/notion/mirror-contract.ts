/**
 * `@docket/connections` — the Notion mirror: Docket-designed databases inside a Notion workspace.
 *
 * @remarks
 * The **inverse** of the generic linked-database connector, which remains in
 * `@docket/integrations`. There, a Notion database already exists and Docket derives a mapping
 * from whatever schema it happens to declare. Here, the user designs the database in Docket —
 * names it, names its columns, chooses how each field is represented — and Docket provisions
 * exactly that, then keeps it current.
 *
