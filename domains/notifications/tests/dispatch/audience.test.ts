...
describe('expandNotificationAudience', () => {
  ...
  it('expands an organization to active human members only', async () => {
    ...
    await addMember(db, schema, orgId, activeUserId);
    await addMember(db, schema, orgId, suspendedUserId, { status: 'suspended' });
    await addMember(db, schema, outsideOrgId, outsideUserId);
  ...
  describe('segment: trial_users', () => {
    it('includes only active members of trialing organizations', async () => {
      ...
      await addMember(db, schema, trialingOrgId, trialUserId);
      await addMember(db, schema, activeOrgId, activeOrgUserId);
  ...
  describe('segment: billing_admins', () => {
    it('expands through role-backed active owner/admin/billing_admin memberships', async () => {
      ...
      await addMember(db, schema, orgId, ownerUserId, { roleKey: 'owner' });
      await addMember(db, schema, orgId, memberUserId);
      await addMember(db, schema, orgId, suspendedOwnerUserId, {
        roleKey: 'owner',
        status: 'suspended',
      });
