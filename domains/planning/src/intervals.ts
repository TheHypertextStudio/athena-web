/**
 * `@docket/planning` — the interval algebra the weekly planner allocates against.
 *
 * @remarks
 ...
 */
import type { AvailabilityWindowKind } from './contracts/scheduling';
...
/** Remove a sorted blocker set from one span. */
function subtractSpan(span: Span, cuts: readonly Interval[]): Span[] {
  const remaining: Span[] = [];
  let cursor = span.start;
  for (const cut of cuts) {
    if (cut.end <= cursor) continue;
    if (cut.start >= span.end) break;
    if (cut.start > cursor) remaining.push({ ...span, start: cursor, end: cut.start });
    cursor = Math.max(cursor, cut.end);
    if (cursor >= span.end) break;
  }
  if (cursor < span.end) remaining.push({ ...span, start: cursor, end: span.end });
  return remaining;
}
...
export function subtractIntervals(spans: readonly Span[], blockers: readonly Interval[]): Span[] {
  ...
  return spans.flatMap((span) => subtractSpan(span, cuts)).sort((a, b) => a.start - b.start);
}

interface SpanTakeOptions {
  readonly kind?: AvailabilityWindowKind;
...
/** Return the exact placement inside one span when every selection condition holds. */
function placementInSpan(
  span: Span,
  needed: number,
  reserve: number,
  trailing: number,
  options: SpanTakeOptions,
): SpanPlacement | null {
  if (options.kind !== undefined && span.kind !== options.kind) return null;
  if (options.date !== undefined && span.date !== options.date) return null;
  if (options.excludeDates?.has(span.date) === true) return null;
  const start = Math.max(span.start, options.notBefore ?? span.start);
  if (start + needed > span.end) return null;
  if (trailing > 0 && start + needed + reserve + trailing > span.end) return null;
  return { start, consumedEnd: Math.min(span.end, start + needed + reserve) };
}
...
export class SpanPool {
   ...
   */
  take(minutes: number, options: SpanTakeOptions = {}): Span | null {
    ...
    for (let i = 0; i < this.#spans.length; i += 1) {
      ...
      const placement = placementInSpan(span, needed, reserve, trailing, options);
      ...
      const taken: Span = {
        ...
        end: placement.start + needed,
      };
      this.#replace(i, span, placement.start, placement.consumedEnd);
      return taken;
    }
    return null;
  }
