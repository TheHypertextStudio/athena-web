...
/** Add assertion provenance to one exact interval. */
function withAssertionProvenance(
  assertion: ResolutionAssertion,
  occurrenceDate: string,
  placeId: string,
  interval: { start: Date; end: Date; timed: boolean },
): AssertionInterval {
  return {
...
/** Return one one-off assertion when it intersects the requested window. */
function oneOffAssertionIntervals(
  assertion: ResolutionAssertion,
  schedule: Extract<WorkLocationSchedule, { type: 'one_off_all_day' | 'one_off_timed' }>,
  windowStart: Date,
  windowEnd: Date,
  ...
  if (interval.end <= windowStart || interval.start >= windowEnd) return [];
  const occurrenceDate =
    schedule.type === 'one_off_all_day'
      ? schedule.date
      : localDateString(interval.start, schedule.timezone);
  return [withAssertionProvenance(assertion, occurrenceDate, assertion.placeId, interval)];
}

/** Expand replacement exceptions that intersect the requested window. */
function replacementIntervals(
  assertion: ResolutionAssertion,
  windowStart: Date,
  windowEnd: Date,
): AssertionInterval[] {
  return assertion.exceptions.flatMap((exception) => {
    if (exception.action !== 'replace') return [];
    const interval = oneOffInterval(exception.schedule);
    if (interval.end <= windowStart || interval.start >= windowEnd) return [];
    return [withAssertionProvenance(assertion, exception.date, exception.placeId, interval)];
  });
}

/** Whether one local date is an active, non-excepted occurrence of a weekly schedule. */
function isWeeklyOccurrenceDate(
  assertion: ResolutionAssertion,
  schedule: Extract<WorkLocationSchedule, { type: 'weekly_all_day' | 'weekly_timed' }>,
  date: string,
): boolean {
  if (compareCalendarDates(date, schedule.effectiveFrom) < 0) return false;
  if (schedule.effectiveUntil !== null && compareCalendarDates(date, schedule.effectiveUntil) > 0) {
    return false;
  }
  if (!schedule.weekdays.includes(mondayWeekdayIndex(date))) return false;
  return !assertion.exceptions.some((candidate) => candidate.date === date);
}

/** Convert one weekly local date to its exact interval. */
...
/** Expand ordinary weekly occurrences that intersect the requested window. */
function weeklyIntervals(
  ...
  return datesForWindow(windowStart, windowEnd, schedule.timezone).flatMap((date) => {
    if (!isWeeklyOccurrenceDate(assertion, schedule, date)) return [];
    const interval = weeklyInterval(schedule, date);
    if (interval.end <= windowStart || interval.start >= windowEnd) return [];
    return [withAssertionProvenance(assertion, date, assertion.placeId, interval)];
  });
}
...
function assertionIntervals(
  ...
  if (schedule.type === 'one_off_all_day' || schedule.type === 'one_off_timed') {
    return oneOffAssertionIntervals(assertion, schedule, windowStart, windowEnd);
  }
  ...
  return [
    ...replacementIntervals(assertion, windowStart, windowEnd),
    ...weeklyIntervals(assertion, schedule, windowStart, windowEnd),
  ];
}
...
/** Convert one active location-bound block into an expected-location result. */
function locatedBlockResolution(
  block: ResolutionWorkBlock,
  state: WorkLocationResolutionState,
): ExpectedResolution | null {
  if (!block.placeId) return null;
  const place = placeSummary(state, block.placeId);
  if (!place) return null;
  return {
    place,
    source: 'work_block',
...
/** Resolve the conservative same-place gap between adjacent blocks. */
function bridgedBlockResolution(
  at: Date,
  state: WorkLocationResolutionState,
  ordered: readonly ResolutionWorkBlock[],
): ExpectedResolution | null {
...
function resolveWorkBlock(at: Date, state: WorkLocationResolutionState): ExpectedResolution | null {
  ...
  if (locatedActive) return locatedBlockResolution(locatedActive, state);

  // Any active unlocated block is real intervening work and makes a location gap ambiguous.
  if (activeBlocks.length > 0) return null;
  return bridgedBlockResolution(at, state, ordered);
}
...
/** Append or coalesce one resolved range fragment. */
function appendResolutionSegment(
  segments: WorkLocationRangeOut['segments'],
  startMs: number,
  endMs: number,
  state: WorkLocationResolutionState,
): void {
  ...
  const resolution = resolveExpectedWithProvenance(new Date(startMs), state);
  const previous = segments.at(-1);
  if (previous?.effectiveEnd === start && sameResolution(previous, resolution)) {
    previous.effectiveEnd = end;
    return;
  }
  segments.push({
    ...resolution,
...
export function resolveExpectedWorkLocationRange(input: {
  ...
  const ordered = resolutionBoundaries(input);
  ...
  for (let index = 0; index < ordered.length - 1; index += 1) {
    ...
    appendResolutionSegment(segments, startMs, endMs, input.state);
  }
