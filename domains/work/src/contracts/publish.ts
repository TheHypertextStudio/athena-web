/**
 * `domain packages` — publishing DTOs: briefs and workspace custom domains.
 *
 * @remarks
 ...
 * (`organization.slug` in the API organization contract) — every workspace has one from the moment
...
import { WorkEntityId, WorkPublicDomainId, WorkPublicationId } from '../ids';
import type { ActorId } from '@docket/identity-access/ids';
...
export const PublicationCreate = z
  .object({
    subjectKind: PublicationSubjectKind,
    subjectId: WorkEntityId.describe('The record to publish; must live in the path organization.'),
