/**
 * The public URL path-segment shape.
 *
 * @remarks
 * One rule set for every string that ends up as a segment in a URL someone else reads: a
 * workspace's own identity slug ({@link OrgCreate}/{@link OrgUpdate}, the API organization contract)
 * and a published brief's own path segment ({@link PublicationCreate}/{@link PublicationUpdate},
 * {@link PublishCreate}). Work owns the shared value so publishing and organization routes do not
