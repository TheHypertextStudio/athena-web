/**
 * `domain packages` — the cycle default-naming scheme.
 *
 * @remarks
...
import { CycleOut, defaultCycleName } from '../../src/contracts/cycle';

/**
 * The host's mutable environment, reached without depending on Node's type definitions.
 *
 * @remarks
 * `domain packages` is a platform-neutral DTO package and deliberately carries no `@types/node`, so
...
describe('defaultCycleName', () => {
  ...
  it('names the window by its UTC calendar days even in a zone behind UTC', async () => {
    ...
    vi.resetModules();
    try {
      const fresh = await import('../../src/contracts/cycle');
