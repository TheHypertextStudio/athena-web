/**
 * `@docket/billing` — idempotent Stripe desired-state provisioning for Docket Pro.
 *
 * @remarks
 * The integration bootstrap calls this module in Stripe test mode first and live mode second.
 * Product, price, customer portal, and webhook resources are reconciled from one declaration;
 * operators never create billing objects by hand or run a release-specific script.
 */
import Stripe from 'stripe';

import { STRIPE_API_VERSION } from './stripe';

/** Stable product key used in Docket and Stripe metadata. */
...
/** Desired product fields that bootstrap owns. */
...
/** Product fields used to identify and reconcile an existing Stripe object. */
export interface StripeProductState extends StripeProductInput {
  readonly id: string;
  readonly livemode: boolean;
}

/** Desired recurring price fields that bootstrap owns. */
...
/** Price fields used to identify and reconcile an existing Stripe object. */
...
/** Desired customer-portal behavior for Docket Pro. */
...
/** Portal fields used to identify and reconcile an existing Stripe object. */
...
/** Webhook fields used to identify and reconcile an existing Stripe object. */
...
/**
 * Narrow Stripe provisioning port.
 *
 * @remarks
 * Keeping the reconciler on a small port makes desired-state and rotation behavior deterministic
 * in tests while the production adapter below remains a direct translation to the official SDK.
 */
 ...
 *
 * @param client - Narrow Stripe mutation port.
 * @param input - Mode and public Docket origins for this environment.
 * @returns Runtime values to persist through the normal integration writers.
 */
export async function reconcileDocketStripe(
  ...
  const productInput: StripeProductInput = {
    ...
    active: true,
    metadata: {
      ...managedMetadata('product'),
      docket_product_key: DOCKET_PRO_PRODUCT_KEY,
    },
  };
  ...
  const desiredPriceInput: StripePriceInput = {
  ...
  const lookupPrices = state.prices.filter(
    (value) => value.lookupKey === DOCKET_PRO_MONTHLY_LOOKUP_KEY && value.active,
  );
  const existingPrice = only(lookupPrices, 'Docket Pro monthly price');
  let price: StripePriceState;
  if (!existingPrice) {
    price = await client.createPrice(desiredPriceInput);
    actions.push('created Docket Pro monthly price');
  } else if (!matchingPrice(existingPrice, product.id)) {
    price = await client.createPrice({ ...desiredPriceInput, transferLookupKey: true });
    ...
    price = await client.updatePrice(existingPrice.id, {
      active: true,
      nickname: desiredPriceInput.nickname,
      metadata: desiredPriceInput.metadata,
    });
  }

  const portalInput: StripePortalConfigurationInput = {
    name: 'Docket Pro billing',
    defaultReturnUrl: `${webOrigin}/settings/billing/return`,
  ...
  const webhookInput: StripeWebhookInput = {
    ...
    description: 'Docket billing (managed by pnpm bootstrap)',
  ...
  if (!existingWebhook) {
    ...
    await client.updateWebhookEndpoint(existingWebhook.id, {
      ...webhookInput,
      status: 'enabled',
    });
 ...
 *
 * @param input - Mode-matched Stripe credential and environment origins.
 * @returns Runtime values for the standard environment writers.
 */
export async function provisionDocketStripe(
  ...
  const apiVersion = STRIPE_API_VERSION as StripeOptions['apiVersion'];
