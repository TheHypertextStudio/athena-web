/**
 * `@docket/boundaries/real` — the env-driven {@link Mailer} adapters.
 *
 * @remarks
 * Two real transports live here, both env-driven and selected only outside
 * `APP_MODE ∈ {local,test}` (see {@link selectAdapter}); neither holds business logic
 * (`boundaries.md` §6):
 *
 * - {@link RealMailer} — posts each message to an HTTP transactional-email provider
 *   (a JSON `POST` with a bearer token). This is the transport `selectAdapter` wires
