...
ALTER TABLE "calendar_layer" ADD COLUMN "removed_at" timestamp;--> statement-breakpoint
ALTER TABLE "calendar_list" ADD COLUMN "removed_at" timestamp;--> statement-breakpoint
...
WHERE
	"source_kind" = 'provider_calendar'
	AND "provider" IS NOT NULL
	AND "external_layer_id" IS NOT NULL;--> statement-breakpoint

-- Google iCalUID identifies the event across calendars. For recurring series it is only safe when
-- originalStartTime also identifies the occurrence. Rows without both anchors remain source-local.
