...
export const organizationProductEntitlement = pgTable(
  ...
  (t) => [
    primaryKey({ columns: [t.organizationId, t.productKey] }),
    index('organization_product_status_idx').on(t.productKey, t.status),
    index('organization_product_subscription_idx').on(t.stripeSubscriptionId),
  ],
);
