...
import { findVar, isRealValue, VAR_REGISTRY } from './index';
import {
  agentServer,
  authServer,
  clientShared,
...
describe.each([
  ['web', () => import('./web')] as const,
  ['marketing', () => import('./marketing')] as const,
  ['admin', () => import('./admin')] as const,
...
describe('api composition', () => {
  ...
  it('throws fail-fast when a required var is missing', async () => {
    ...
    await expect(import('./api')).rejects.toThrow('Invalid environment variables');
  });
