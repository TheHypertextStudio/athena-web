...
export const CORE_VARS: readonly VarSpec[] = [
  ...
  },
  {
    ...
    where:
      'Previous WebAuthn relying-party id accepted by the temporary native migration ceremony and its web trust-document redirect exception',
    sensitive: false,
  },
  {
  ...
  {
    name: 'ADMIN_GOOGLE_SSO_ENABLED',
    slice: 'auth',
    scope: 'server',
    targets: ['api'],
    required: true,
    zod: authServer.ADMIN_GOOGLE_SSO_ENABLED,
    where: 'true once the Workspace groups exist and the runtime service account can read them',
  },
  {
    name: 'ADMIN_GOOGLE_GROUP_ROLES',
    slice: 'auth',
    scope: 'server',
    targets: ['api'],
    required: false,
