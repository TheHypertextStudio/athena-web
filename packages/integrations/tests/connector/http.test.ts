 ...
 * `HttpClient` is injected. Mirrors `@docket/mail`'s equivalent test: forwards to the global
 * `fetch` verbatim, and fails clearly rather than with a cryptic `TypeError` when no global
 * `fetch` exists.
 */
