 ...
 *
 * @remarks
 * An **identity** is an external account (today, a Google account) the user has linked to their
...
/**
 * A linked external identity (a Google account the user authorized).
 *
 * @remarks
 * `email`/`name`/`picture` come from the stored OIDC `id_token` and may be absent (the token can
 * lack a claim or be unavailable), so they are nullable. `accountId` is the provider `sub` — the
 * stable id an org integration binds to as its `externalAccountId`.
 */
export const IdentityOut = z
  .object({
    /** The provider account id (Google `sub`); an integration's `externalAccountId`. */
    accountId: z.string(),
  ...
  .meta({ id: 'IdentityOut', description: 'A linked external identity (e.g. a Google account).' });
