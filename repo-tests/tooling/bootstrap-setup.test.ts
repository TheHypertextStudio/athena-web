...
describe('bootstrap phase flags', () => {
  ...
  it('seeds every required local feature control with its safe default', () => {
    ...
    expect(example).toContain('WORK_LOCATION_PROJECTION_ENABLED=false');
    expect(example).toContain('BILLING_CANARY_EMAILS=');
    expect(example).toContain('BILLING_RECONCILIATION_MODE=off');
  });

  it('delegates local configuration to the registry-backed reconciler', () => {
    const bootstrap = readFileSync(
      resolve(import.meta.dirname, '../../scripts/bootstrap.ts'),
      'utf8',
    );

    expect(bootstrap).toContain("from './bootstrap/local-config'");
    expect(bootstrap).toContain('reconcileLocalConfig({');
    expect(bootstrap).not.toContain('PORT=3001');
    expect(bootstrap).not.toContain('const content = `# Generated by pnpm bootstrap');
  });
});
