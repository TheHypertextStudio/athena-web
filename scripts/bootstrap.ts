...
export function writeEnvLocal(): void {
  ...
  if (result.diagnostics.length > 0) {
    ...
    throw new Error(
      `.env.local has ${String(result.diagnostics.length)} unresolved configuration problem(s)`,
    );
  }
  ok(result.changed ? '.env.local reconciled' : '.env.local already converged');
}
