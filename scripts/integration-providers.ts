...
export interface ProviderGroup {
  ...
  /** Values produced by a managed provisioner instead of typed by the operator. */
  readonly managedVars?: readonly string[];
  /** Reusable provisioner invoked by the integration orchestrator. */
...
export const PROVIDER_GROUPS: readonly ProviderGroup[] = [
  ...
  },
  {
    ...
    steps: (env, urls) => {
      ...
      return [
        ...
        {
          note: [
            'Find the "Identifying and authorizing users" section — this is how sign-in returns',
            'the user to Docket. This is browser-facing, so it goes',
            'on the product origin(s), not the API host.',
            '',
            'In "Callback URL", add the sign-in callback for every Docket frontend:',
            '',
            ...urls.webBases.map((web) => `  • ${web}/api/auth/callback/github`),
            '',
            ...
            '',
            'Under "Post installation", clear "Request user authorization (OAuth) during',
            'installation". Then set the Setup URL to:',
            '',
            ...urls.webBases.map((web) => `  • ${web}/internal/integrations/github/callback`),
            '',
            'and select:',
            '',
            '  • "Redirect on update" under the Post installation section',
            '',
            "The Setup URL receives GitHub's installation id after the person chooses an account",
            'and repositories. It is separate from the sign-in callback above.',
          ],
        },
        {
          note: [
            'Create the app now. At the bottom, under "Where can this GitHub App be installed?",',
            'choose "Any account", then click the big green "Create GitHub App" button. This',
            'lets each workspace choose the GitHub account or organization and repositories it',
            'wants Docket to access during the one-click connection ceremony.',
            '',
  ...
  },
  {
    ...
    instructions: (env, urls) => [
      ...
      '3) Redirect URIs — add one per Docket frontend, exactly, no trailing slash:',
      ...urls.webBases.map((web) => `     ${web}/api/auth/callback/notion`),
      '4) Create → "Configuration" tab → check all three Capabilities (Read/Update/Insert',
  ...
  },
  {
    ...
    policyVars: ['APPLE_APP_CLIENT_ID'],
    cloudVariables: ['APPLE_APP_CLIENT_ID'],
    instructions: (_env, urls) => [
      'Adds "Sign in with Apple" to the web and native clients. ~10 min. You need an Apple',
      ...
      '2) Create an App ID (skip if Docket already has one): "+" → "App IDs" → "App" → use',
      '   `studio.hypertext.docket` as the Bundle ID, tick "Sign in with Apple", make it the',
      '   primary App ID, and enter the same identifier as APPLE_APP_CLIENT_ID below.',
      '3) Create a Services ID: "+" → "Services IDs" → Description: "Docket Web" → Identifier',
      '   `studio.hypertext.docket.web` (this becomes APPLE_CLIENT_ID) → Continue → Register.',
      '4) Open that Services ID → tick "Sign in with Apple" → "Configure":',
      '     • Primary App ID: the App ID from step 2.',
      '     • Domains and Subdomains: every bare web and API hostname used below.',
      '     • Return URLs — register the web proxy and direct API callback, exactly:',
      ...urls.webBases.map((web) => `         ${web}/api/auth/callback/apple`),
      `         ${urls.apiBase}/api/auth/callback/apple`,
      ...
      '8) Note the Key ID shown on that same page (10 characters) → that is APPLE_KEY_ID. Keep the',
      '   downloaded file outside the repository until this wizard confirms its Secret Manager copy.',
      '',
    ],
    ...
    instructions: (env, urls) => {
      ...
      return [
        ...
        ...(env === 'production'
          ? [
              '',
              'Production is the second pass. Configure local first so the same desired state is',
              'created and verified in Stripe test mode before live mode.',
            ]
          : [
              '',
              'Sandbox webhooks use Stripe CLI signed forwarding; no public tunnel is required.',
              'Run the local API and Stripe listener while exercising checkout.',
            ]),
      ];
    },
  },
